...
// newTwoToolBackend creates a mock HTTP backend that returns two tools: read_file and write_file.
func newTwoToolBackend(t *testing.T) *httptest.Server {
	t.Helper()
			...
			json.NewEncoder(w).Encode(map[string]interface{}{
				...
				"result": map[string]interface{}{
					"tools": []map[string]interface{}{
						...
						{"name": "write_file", "description": "Write a file", "inputSchema": map[string]interface{}{"type": "object"}},
					},
				},
			})
		}
...
// TestRegisterToolsFromBackend_ToolsFilter_AllowsOnlyListed tests that when Tools is
// configured, only the listed tools are registered.
func TestRegisterToolsFromBackend_ToolsFilter_AllowsOnlyListed(t *testing.T) {
	assert := assert.New(t)
	require := require.New(t)

	backend := newTwoToolBackend(t)
	defer backend.Close()
	...
	cfg := &config.Config{
		Servers: map[string]*config.ServerConfig{
			"filter-backend": {
				Type:  "http",
				URL:   backend.URL,
				Tools: []string{"read_file"}, // only allow read_file
			},
		},
	}
	...
	err = us.registerToolsFromBackend("filter-backend")
	require.NoError(err)

	us.toolsMu.RLock()
	defer us.toolsMu.RUnlock()

	assert.Len(us.tools, 1, "Only 1 tool should be registered")
	assert.NotNil(us.tools["filter-backend___read_file"], "read_file should be registered")
	assert.Nil(us.tools["filter-backend___write_file"], "write_file should be filtered out")
}

// TestRegisterToolsFromBackend_ToolsFilter_EmptyAllowsAll tests that when Tools is
// empty/nil, all tools from the backend are registered.
func TestRegisterToolsFromBackend_ToolsFilter_EmptyAllowsAll(t *testing.T) {
	assert := assert.New(t)
	require := require.New(t)
	...
	cfg := &config.Config{
		Servers: map[string]*config.ServerConfig{
			"nofilter-backend": {
				Type:  "http",
				URL:   backend.URL,
				Tools: nil, // no filter — all tools should be exposed
			},
		},
	}
	...
	assert.Len(us.tools, 2, "Both tools should be registered when no filter is set")
	assert.NotNil(us.tools["nofilter-backend___read_file"])
	assert.NotNil(us.tools["nofilter-backend___write_file"])
}

// TestRegisterToolsFromBackend_ToolsFilter_NoneMatchYieldsEmpty tests that when the
// Tools filter lists names that don't match any backend tool, no tools are registered.
func TestRegisterToolsFromBackend_ToolsFilter_NoneMatchYieldsEmpty(t *testing.T) {
	assert := assert.New(t)
	require := require.New(t)
	...
	require.NoError(err, "Should succeed even if no tools match the filter")

	us.toolsMu.RLock()
	defer us.toolsMu.RUnlock()

	assert.Empty(us.tools, "No tools should be registered when filter matches nothing")
}
