...
// TestTagsToStrings_Reflect verifies the private tagsToStrings helper in
// reflect.go directly, confirming it converts Tag slices to *sorted* string
// slices (unlike the exported TagsToStrings which preserves order).
func TestTagsToStrings_Reflect(t *testing.T) {
	...
	t.Run("empty slice returns empty slice", func(t *testing.T) {
		result := tagsToStrings([]Tag{})
		require.NotNil(t, result)
		assert.Empty(t, result)
	})

	t.Run("single tag is converted to string", func(t *testing.T) {
		result := tagsToStrings([]Tag{"private:owner/repo"})
		require.Len(t, result, 1)
		assert.Equal(t, "private:owner/repo", result[0])
	})

	t.Run("multiple tags are sorted alphabetically", func(t *testing.T) {
		tags := []Tag{"zebra", "alpha", "mango", "apple"}
		result := tagsToStrings(tags)
		require.Len(t, result, 4)
		assert.Equal(t, []string{"alpha", "apple", "mango", "zebra"}, result)
	})

	t.Run("already-sorted tags remain sorted", func(t *testing.T) {
		tags := []Tag{"aaa", "bbb", "ccc"}
		result := tagsToStrings(tags)
		assert.Equal(t, []string{"aaa", "bbb", "ccc"}, result)
	})

	t.Run("tags with colons sort by full string value", func(t *testing.T) {
		tags := []Tag{"repo:org/z", "private:org/a", "merged:all"}
		result := tagsToStrings(tags)
		require.Len(t, result, 3)
		assert.True(t, sort.StringsAreSorted(result),
			"result should be sorted; got %v", result)
	})

	t.Run("tags with identical values produce duplicate strings", func(t *testing.T) {
		// Tags are unique by value in normal use, but tagsToStrings does not deduplicate.
		tags := []Tag{"tag-a", "tag-a"}
		result := tagsToStrings(tags)
