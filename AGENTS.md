...
- `MCP_GATEWAY_TOOL_TIMEOUT` - Tool invocation timeout in seconds. Fallback when `gateway.toolTimeout` is not set in stdin JSON config. Accepts integers in the range `10`–`600`. Priority: stdin config > env var > built-in default. (default: `60`)
